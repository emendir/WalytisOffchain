...
class PrivateBlockchain(blockstore.BlockStore):
    def __init__(self,
        ...
        self.base_blockchain = base_blockchain_type.__init__(
        ...
        self.content_request_listener = ConversationListener(
            listener_name=f"PrivateBlocks{self.base_blockchain.id}",
    ...
    def get_block_ids(self):
        return self.base_blockchain.get_block_ids()

    def get_blocks(self):
        return [self.load_block(block_id) for block_id in self.base_blockchain.block_ids]

    def add_block(self, content: bytearray, topics=[]):
    ...
    def _on_block_received(self, block):
        ...
        private_block = data_block.DataBlock(block, private_content, author)
        self.block_received_callback(private_block)

    def ask_around_for_content(self, block):
        """
        No Exceptions, while loop until content is found, unless we want to
        keep track of processed blocks ourselves
        instead of letting walytis_api do.
        """
        peers = self.base_blockchain.get_peers()
        # move block creator to top of list
        peers.remove(block.creator)
        peers.insert(0, block.creator)

        private_content: bytearray | None = None
        while not private_content:
            ...
            peers = self.base_blockchain.get_peers()
        return author, self.identity.decrypt(private_content)

    def handle_content_request(self, conv_name, peer_id):
